...
- 2026-09-16 LIFE `home-printer` topic runs failed on `Reconnecting... 2/5` while the bridge treated an app-server `error` notification as terminal. The bridge now reports these notices as progress and waits for `turn/completed`; unexpected app-server exit still fails the run. Commit `b1157f7ee1e21493c1488ce375005c02a2321394` was deployed and diagnostic run `#74` completed with `LIFE CONNECTED`.
